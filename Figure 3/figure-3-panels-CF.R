...
perfect_squares = (4:20)^2
n_units_vec = sample(                               # Number of units per network (varying)
  perfect_squares, n_replicates, replace=T)         
...
ID = 1
df = sapply(n_units_vec, function(n_units) {        # Simulate while number of units is varying
...
g_r_squared_C = ggplot(df_cor, aes(n_units, r_squared, color=Reentry, shape=Reentry)) +
  geom_smooth(aes(fill=Reentry), span=0.25, color=NA, method="loess") +
  geom_point() +
  labs(x="# of units", y=expression("r"^"2"),
...
IDs_to_plot = sapply(                                # Choose low, medium, and high unit examples
  c(64, 121, 225), function(x) df$ID[which.min((df$n_units-x)^2)] )
df_rep = df %>%
...
### Generate graphic
g_representative_F = ggplot(df_rep, aes(row, col, fill=`Firing rate`)) +
  facet_grid(Reentry~n_units, switch="y") +
  geom_raster() +
...
## Arrange plots
grid.arrange(g_r_squared_C, g_representative_F, nrow=1)
