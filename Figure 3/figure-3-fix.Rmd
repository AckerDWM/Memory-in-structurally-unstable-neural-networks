...
# this alternate version of the reentry function 
# initializes the network in the vicinity of the attractor
reenter_positive = function(net, I) {
  n_units = nrow(net$weights)
  correlated_input = .001*(I + runif(n_units, -2, 2))
  net$membrane_potentials = correlated_input %>% matrix(1, n_units)
  net$firing_rates = tanh(net$membrane_potentials)
  for (i in 1:12) {
    X = net$firing_rates
    net$membrane_potentials = np$dot(X, net$weights)
...
g_pconnection = df_result_pconnection %>%
  ...
  labs(x="Probability of connection", y="r")
g_pconnection
```
...
g_indegree = df_result_indegree %>%
  ...
  labs(x="Median in-degree", y="Turnover rate [%]", color="r")
g_indegree

df_result_indegree %>%
  filter(r >= .9, turnover_rate>=.8, reactivation==100)
```
...
g_rep_turnover = ggplot(df_rep, aes(Var1, Var2, fill=value)) +
  ...
  geom_raster() +
  scale_fill_distiller(palette="Greys", direction=1, breaks=c(-1, 1), labels=c("Min.", "Max.")) +
  coord_equal() +
  theme_classic() +
  theme(
    axis.line = element_blank(),
...
simulation_dVm = function(n_units, turnover_rate, p_connection) {
  ...
  for (i in 1:5) {
    ...
    membrane_potentials = rbind(membrane_potentials, net$membrane_potentials_all_timesteps) # save
  }
  
  # Gather results
  result_list = list(
    firing_rates = membrane_potentials
    )
...
results_list = simulation_dVm(n_units=n_units, turnover_rate=.73, p_connection=.2)
...
g_attractor_dynamics = bind_rows(df_stable, df_unstable, df_no_turnover) %>%
  ...
  labs(x="Time step", y=TeX("$\\Delta$ u"))
...
tiff("figure-3-in-basin.tiff", width=6.18, height=5, units="in", res=300)
figure
dev.off()
```
